...
export function gardenMove(g:Garden,who:number,verb:string,cell:number,seed:string,now:number){
 ...
 if(verb==='plant'){if(plot)throw Error('This plot is already planted.');const type=seeds.find(s=>s.id===seed);if(!type)throw Error('Choose a seed.');if(type.id==='moon'&&g.level<2)throw Error('Upgrade to level 2 for moonflowers.');if(g.coins<type.cost)throw Error('Harvest a flower to earn more petals.');g.coins-=type.cost;g.plots[cell]={seed,planted:now,readyAt:now+Math.round(type.grow*(1-(g.level-1)*.15)*(g.weather==='rain'?.7:1)),watered:false};g.log=`${type.name} planted in plot ${cell+1}.`;}
