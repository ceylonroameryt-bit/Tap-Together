...
export function fishMove(p:Angler,value:string,now:number){
 ...
 else{const fish=fishTypes[(p.casts-1)%3];p.catches++;p.combo++;p.points+=fish.points+Math.min(3,Math.floor(p.combo/3));if(!p.collection.includes(fish.name))p.collection.push(fish.name);p.last=`Caught a ${fish.name}! ${p.combo>1?`${p.combo} catch combo!`:''}`;}
